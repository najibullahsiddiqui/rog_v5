...
class AdminStore:
    ...
    def _get_or_create_chat_session(self, conn: sqlite3.Connection, session_key: str) -> int | None:
        if not self._table_exists(conn, "chat_sessions"):
            return None

        safe_key = (session_key or "").strip() or "default_session"
        conn.execute(
            """
            INSERT INTO chat_sessions (session_key, channel, status, started_at, updated_at)
        ...
        row = conn.execute(
            "SELECT id FROM chat_sessions WHERE session_key=? LIMIT 1",
            (safe_key,),
        ).fetchone()
        return int(row["id"]) if row else None

    def _find_source_document_id_by_name(self, conn: sqlite3.Connection, doc_name: str | None) -> int | None:
        if not doc_name or not self._table_exists(conn, "source_documents"):
            return None

        value = str(doc_name).strip()
        if not value:
            return None
    ...
    def log_chat_interaction(
        ...
        *,
        ...
        with self._conn() as conn:
            ...
            session_id = self._get_or_create_chat_session(conn, session_key)
            if not session_id:
                return None

            category_id = None
            if category_code and self._table_exists(conn, "categories"):
                row = conn.execute(
                    "SELECT id FROM categories WHERE code=? AND is_active=1 LIMIT 1",
                    (category_code,),
                ).fetchone()
                if row:
            ...
            if confidence is not None:
                evidence_payload.insert(0, {"confidence": confidence})
            ...
            if self._table_exists(conn, "message_citations") and isinstance(citations, list):
                for citation in citations:
                    if not isinstance(citation, dict):
                        continue
                    source_document_id = self._find_source_document_id_by_name(
                        conn,
                        str(citation.get("doc_name") or ""),
                    )
                    conn.execute(
                        """
